    ...
    <button onClick={removeCommunity}>Test remove community</button> */}
        {reactions.reactionsStatistics.length && (
            <div>
                {reactions.reactionsStatistics.map((emoji, index) => {
                    return (
                        <div key={index}>
                            Emoji: {emoji.emoji}. Total accounts:{' '}
                            {emoji.accounts.length}. Accounts:{' '}
                            {JSON.stringify(emoji.accounts)}
                        </div>
                    )
                })}
            </div>
        ...
        {reactions.reactionsStatistics.length && (
            <div>
                {' '}
                Total reactions:
                {reactions.reactionsStatistics.reduce(
                    (totalReactions, currEmoji) => {
