const renderFeedback = (functionArray, filteredErrorResults, functionsPassingTest, isSync) => {
  return <>
    <strong>Running {`${isSync ? "sync": "async"}`} functions: </strong>
    <ul>
          ...
          )}
          {
            <div>
              {Object.keys(filteredErrorResults).map((fn) => {
                return (
                  <div>
                    <strong className="text-danger">{`${fn}`}:</strong>
...
function displayTestsSyncResults(functionArray) {
  ...
  return renderFeedback(functionArray, filteredErrorResults, functionsPassingTest, true);
}
...
function displayTestsAsyncResults(functionArray) {
  ...
  return finalResults.then((result) => {
    ...
    return renderFeedback(functionArray, filteredErrorResults, functionsPassingTest, false);
  });
}
